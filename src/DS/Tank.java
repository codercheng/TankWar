...
abstract class Tank
{
	...
	public double TANKSPEED=5;  //Tank移动速度
	...
	public static final int TURRETLENGTH=15; //炮筒的长度
	...
	public void changeSpeed(){
		TANKSPEED=2.5+this.tankHitPoint.number/10.0;
	}
	...
	{
		isLive=false;
		this.tankHitPoint.number=0;
			
		if((this instanceof BossRobotTank) && (!this.camp))
			switch(rn.nextInt(12))
			{case 0:
			    tc.itemList.add(new moneyItem(x,y,100,tc));break;
			case 1:
				tc.itemList.add(new HitPointItem(x,y,10,tc));break;
			case 2:
				tc.itemList.add(new ShotsItem(x,y,0,tc));break;
			case 3:
				tc.itemList.add(new ShotsItem(x,y,1,tc));break;
			case 4:
				tc.itemList.add(new freezeItem(x,y,5000,tc));break;
			case 5:
				tc.itemList.add(new shieldItem(x,y,tc));break;
			case 6:
			case 8:
				tc.itemList.add(new speedItem(x,y,0,tc));break;
			case 7:
			case 9:
				tc.itemList.add(new speedItem(x,y,1,tc));break;
			case 10:
				tc.itemList.add(new keyItem(x,y,tc));break;
			default:
					break;
				
			}
		
		else if((this instanceof BrainRobotTank) && (!this.camp))
		{
			switch(rn.nextInt(7))
			{
			case 0:
			case 1:
				break;
			case 2:
				tc.itemList.add(new HitPointItem(x,y,8,tc));break;
			case 3:
				tc.itemList.add(new freezeItem(x,y,3000,tc));break;
			case 4:
				tc.itemList.add(new shieldItem(x,y,tc));break;
				default:break;
			}
		}
		else if((this instanceof RobotTank) && (!this.camp))
		{
			switch(rn.nextInt(6))
			{
			case 0:break;
...
class RobotTank extends Tank  //电脑控制的Tank
{
	...
	public static void add(int count,boolean camp,RTANKTYPE type,TankClient tc,int x,int y)  //添加机器人Tank
	{
		if(count>20) { count=20; } //一次添加的数目不能多于20
		...
		for(int i=0;i<count;)
		{
			...
			switch(type)
			{
				case NOR:{tempTank=new RobotTank(x,y,camp,tc);}break;
				case SPE:{tempTank=new BrainRobotTank(x,y,camp,tc);}break;
				default:{tempTank=new BossRobotTank(x,y,camp,30,tc);}break;
			}
	...
	public void hitDispose()  //处理碰撞
	{
		...
		for(int j=0;j<tc.tanksList.size();j++)  //碰撞检测
		{
			...
			if(this.getRect().intersects(tempTank.getRect())) //和别的Tank碰撞 
			{
				...
				{
					if(!tempTank.isNPC&&((UserTank)tempTank).shieldNum.ShieldNumber>0)
						((UserTank)tempTank).shieldNum.cutsShieldNumber(1);
					else{
					tempTank.tankHitPoint.cutsHitPoint(1);}
					this.tankHitPoint.cutsHitPoint(1);
					tempTank.changeSpeed();
					this.changeSpeed();
				}
			}
		}
...
class BrainRobotTank extends RobotTank  //智能较高的机器人Tank
{
	...
	{
		super(x, y, camp, tc);
		this.tankHitPoint=new HitPoint(15,this);
	}
...
class UserTank extends Tank  //由玩家自主控制的Tank
{
	...
	money moneyNum;
	shield shieldNum;
	String str="☆★退出游戏请按 ESC / DELETE键★☆";
	boolean doorOpen=false;//?????????????????????????????????
	//Random selectionCount=new Random();
	//int select=0;
	...
	static final Image SHOTS[]={
		...
		TLK.getImage(Tank.class.getClassLoader().getResource("Image/Tank/person.png")),
		TLK.getImage(Tank.class.getClassLoader().getResource("Image/Tank/blood.png")),
		TLK.getImage(Tank.class.getClassLoader().getResource("Image/Tank/money.png")),
		TLK.getImage(Tank.class.getClassLoader().getResource("Image/Tank/shield_1.png")),
		TLK.getImage(Tank.class.getClassLoader().getResource("Image/Tank/speedIcon.png")),
		TLK.getImage(Tank.class.getClassLoader().getResource("Image/Tank/bulletIcon.png")),
		TLK.getImage(Tank.class.getClassLoader().getResource("Image/Tank/bombIcon.png"))
	};
	...
	public UserTank(int x,int y,TANK_DIR dir,boolean camp,TankClient tc)
	{
		...
		moneyNum=new money(100,this);
		shieldNum=new shield(30,this);
	...
	public void draw(Graphics g)
	{
		//select=(select++)%100;
		super.draw(g);
		if(this.shieldNum.ShieldNumber>0)
		this.shieldNum.draw(g);
		showAmmo(g);
		//if(select==0)
		showText(g);
	}
	public void showText(Graphics g){
		...
		g.setColor(Color.decode("#0033FF"));
		g.drawString(str,785,11);
		g.setColor(c);
	}
	...
	{
		this.hydraShotsNum+=10;
	}
	
	public void addSuperShots()
	{
		this.superShotsNum+=5;
	}
	...
	public void showAmmo(Graphics g)
	{
		Color c=g.getColor();
		g.setColor(Color.decode("#0033FF"));
		g.drawString(" "+this.tankHitPoint.number, 67, 25);//@@@@@@@@@@@@@@@@@@@@@@@@@@@@
		g.drawString(" "+this.moneyNum.MoneyNumber, 67, 55);//加钱
		g.drawString(""+this.shieldNum.ShieldNumber,109,24);
		g.drawString(" "+(int)(30+this.TANKSPEED*20)+"km/h", 145, 25);//speed
		g.drawString(""+this.hydraShotsNum,109,54);
		g.drawString(""+this.superShotsNum+"  ★",146,54);
		g.setColor(c);
	//	g.draw
		g.drawImage(SHOTS[8],130,41,null);
		g.drawImage(SHOTS[7],90,41,null);
		g.drawImage(SHOTS[6],130,11,null);//speed
		g.drawImage(SHOTS[5],90,11,null);//shield
		
		g.drawImage(SHOTS[3],48,10,null);
		g.drawImage(SHOTS[4],48,40,null);
		g.drawImage(SHOTS[2],-5,3,null);
	}
	...
	public void hitDispose()
	{
		...
		for(int i=0;i<tc.tanksList.size();i++)  //碰撞检测
		{
			...
			if(this.getRect().intersects(tempTank.getRect())) //和别的Tank碰撞 
			{
				...
				{
					if(this.shieldNum.ShieldNumber>0)
						this.shieldNum.cutsShieldNumber(1);//?????
					else
						this.tankHitPoint.cutsHitPoint(1);
					tempTank.tankHitPoint.cutsHitPoint(1);
					tempTank.changeSpeed();
	...
	{
		if(moneyNum.MoneyNumber>=100){
		moneyNum.cutsMoneyNumber(100);//要100金币
		this.hydraShotsNum=0;//死后丢掉各种炮弹
		this.superShotsNum=0;//
		
		this.isLive=true;
		this.keyList.clear();
		this.setMoveDir(TANK_DIR.STOP);
		Item tempItem=null;
		for(int i=0;i<tc.itemList.size();i++)//
		{
			tempItem=tc.itemList.get(i);
			if(tempItem instanceof shopItem){
				((shopItem) tempItem).open=false;
			}
	...
	public void help(){//还差个条件，不然不就乱套了吧（随机出来助手的等级）
		if(moneyNum.MoneyNumber>=100){
		RobotTank.add(1,true,RobotTank.RTANKTYPE.SPE,tc);
		moneyNum.cutsMoneyNumber(80);}//要80金币
	}
	//@SuppressWarnings("static-access")
	@SuppressWarnings({ "deprecation" })
	public void freeze(long ms)       //        ******************冻结
	{
		tc.threadRobot.suspend();
		Timer time=new Timer();
		time.schedule(new myTask(), ms);	
